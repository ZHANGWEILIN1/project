...
def WGS84toOSGB36(latitude, longitude, radians=False):
    ...
    xyz_coords = lat_long_to_xyz(latitude, longitude, radians=True, datum=wgs84)
    transformed_xyz = WGS84toOSGB36transform(xyz_coords)

    return xyz_to_lat_long(*transformed_xyz, radians=True)
...
def get_easting_northing_from_lat_long(latitude, longitude, radians=False):
    ...
    # Apply Helmert transormation to GPS ellipsoidal coordinates
    latOS, lonOS = WGS84toOSGB36(LAT, LON, radians=True)

    # convert OS ellipsoidal coordinates to OS grid coordinates
    # using the Lee-Redfearn implementation of the Guass-Kruger transverse mercator projection
    n = (osgb36.a - osgb36.b)/(osgb36.a + osgb36.b)
    nu = osgb36.a*osgb36.F_0*(1-osgb36.e2*sin(latOS)**2)**(-1/2)
    rho = osgb36.a*osgb36.F_0*(1-osgb36.e2)*(1-osgb36.e2*(sin(latOS))**2)**(-3/2)
    eta = sqrt(nu/rho-1)

    M = osgb36.b*osgb36.F_0*((1 + n + (5/4)*n**2 + (5/4)*n**3)*(latOS - osgb36.phi_0)
                            - (3*n + 3*n**2 + (21/8)*n**3)*sin(latOS - osgb36.phi_0)*cos(latOS + osgb36.phi_0)
                            + ((15/8)*n**2 + (15/8)*n**3)*sin(2*(latOS - osgb36.phi_0))*cos(2*(latOS + osgb36.phi_0))
                            - ((35/24)*n**3)*sin(3*(latOS - osgb36.phi_0))*cos(3*(latOS + osgb36.phi_0)))

    I = M + osgb36.N_0
    II = nu/2*sin(latOS)*cos(latOS)
    III = (nu/24*sin(latOS)*(cos(latOS))**3)*(5 - (tan(latOS))**2 + 9*eta**2)
    IIIA = (nu/720*sin(latOS)*(cos(latOS))**5)*(61 - 58*(tan(latOS))**2 + (tan(latOS))**4)
    IV = nu*cos(latOS)
    V = (nu/6*(cos(latOS))**3)*(nu/rho - (tan(latOS))**2)
    VI = (nu/120*(cos(latOS))**5)*(5 - 18*(tan(latOS))**2 + (tan(latOS))**4 + 14*eta**2 - (58*(tan(latOS))**2)*eta**2)

    easting = osgb36.E_0 + IV*(lonOS - osgb36.lam_0) + V*(lonOS - osgb36.lam_0)**3 + VI*(lonOS - osgb36.lam_0)**5
    northing = I + II*(lonOS - osgb36.lam_0)**2 + III*(lonOS - osgb36.lam_0)**4 + IIIA*(lonOS - osgb36.lam_0)**6
